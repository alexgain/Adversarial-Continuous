...
parser.add_argument('--widthD1', type = int, default=500)
...
my_netD1 = Net(input_size, widthD1, width)
if loadD1:
    # my_netD.load_state_dict(torch.load('defense_trained.state'))
...
class IFGSM():
    ...
    def forward(self, x, y_true, model, modelD, modelD1):
        x_adv = x
    
        for k in range(self.iters):
            x.requires_grad = True
            # x_adv.requires_grad = True
            y = model.forward1mod(x,modelD1.forward(x))
            y += modelD(x)
...
def test_acc_adv():
    ...
    for images, labels in test_loader_bap:
        ...
        images = adv_attack.forward(images, Variable(labels), my_net, my_netD, my_netD1)
        outputs = my_net(images)
        _, predicted = torch.max(outputs.data, 1)
    ##    labels = torch.max(labels.float(),1)[1]
    ##    predicted = torch.round(outputs.data).view(-1).long()
...
def test_acc_adv_def():
    ...
    for images, labels in test_loader_bap:
        ...
        images = Variable(images.view(-1,28*28), requires_grad=True)
        images = adv_attack.forward(images, Variable(labels), my_net, my_netD, my_netD1)
        outputs = my_net.forward1mod(images,my_netD1(images)) + my_netD(images)
        _, predicted = torch.max(outputs.data, 1)
    ##    labels = torch.max(labels.float(),1)[1]
    ##    predicted = torch.round(outputs.data).view(-1).long()
        total += labels.size(0)
...
##        y = torch.Tensor(to_categorical(y.long().cpu().numpy(),num_classes)) #MSE
        ...
        x_adv = adv_attack.forward(x, Variable(y), my_net, my_netD, my_netD1)

        ###regular BP gradient update:
        if not no_train:
            optimizer.zero_grad()
        outputs1 = my_net.forward1(x)        
        outputs = my_net.forward1mod(x,outputs1)
        outputs_adv1 = my_net.forward1(x_adv)
        outputs_adv = my_net.forward1mod(x_adv,outputs_adv1)
        if not no_train:
            loss = loss_metric(outputs,y)
            loss.backward(retain_graph=True)
            optimizer.step()

        ###Defense network1 update 1:
        optimizerD1.zero_grad()
        outputsD1 = my_netD1(x_adv)        
        lossD1 = ((outputsD1 - (outputs1 - outputs_adv1))**2).mean()
        lossD1.backward(retain_graph=True)

        ###Defense network1 update 2:
        outputsD1 = my_netD1(x)        
        lossD12 = ((outputsD1 - 0)**2).mean()
        lossD12.backward(retain_graph=True)

        optimizerD1.step()
    
        ###Defense network update 1:
        optimizerD.zero_grad()
        outputsD = my_netD.forward(x_adv)        
        lossD = ((outputsD - (outputs - outputs_adv))**2).mean()
        lossD.backward()

        ###Defense network update 2:
        outputsD = my_netD.forward(x)        
